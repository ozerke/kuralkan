...
@php
    ...
    $config = [
        'paging' => true,
        'searching' => true,
        'columns' => [null, ['orderable' => false], null, null, null, null, null, null],
    ];
@endphp
    ...
    <x-adminlte-datatable id="shop-stocks-table" :heads="$heads" :config="$config" beautify striped hoverable
        ...
        @foreach ($shopStocks as $shopStock)
            <tr style="white-space:nowrap;width:100%;">
                ...
                <td>{{ optional($shopStock->shop)->getCity()->currentTranslation->city_name ?? '-' }}</td>
                <td>{{ optional($shopStock->shop)->district->currentTranslation->district_name ?? '-' }}</td>
                <td>
