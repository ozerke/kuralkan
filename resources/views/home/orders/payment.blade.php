        ...
        <form method="POST" action="{{ route('process-payment', ['orderNo' => $order->order_no]) }}">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-10 gap-8 py-[40px] w-full">
                <div class="col-span-1 lg:col-span-7 flex flex-col gap-8 order-2">
                    <h3 class="font-bold text-2xl">{{ __('web.payment') }}</h3>
                    @if (!$isCampaignOrder && $paidState['paid_amount'] > 0)
                        <div
                            ...
                            <p class="text-lg"><span class="font-bold">{{ __('web.remaining-amount') }}:</span> <span
                    ...
                    @if (count($campaigns))
                        @include('home.components.campaign-box-payment')
                    @endif
                    <div class="flex flex-col gap-4 lg:gap-2">
                        ...
                        <div class="flex flex-col lg:flex-row gap-4 lg:gap-8 min-h-[50px] lg:items-center">
                            <x-bladewind.radio-button id="oneshot_selection" checked="true" color="blue"
                                label="₺{{ number_format($isCampaignOrder ? $paidState['campaign_remaining_amount'] : $paidState['remaining_amount'], 2, ',', '.') }}"
                                name="payment_amount" value="full" labelCss="!mb-0 !text-[18px] font-bold" />
                            @if (!$isCampaignOrder)
                                <div class="flex flex-col lg:flex-row gap-2">
                                    <x-bladewind.radio-button id="partial_selection" color="blue"
                                        label="{{ __('web.partial-amount') }}" name="payment_amount" value="partial"
                        ...
                        <div class="flex flex-col gap-8">
                            <div class="flex flex-col gap-4">
                                ...
                                <div id="credit-card-option" class="flex flex-col bg-white rounded-md shadow-md p-4">
                                    <input hidden id="order-id" value="{{ $order->id }}" />
                                    @if ($isCampaignOrder)
                                        <input hidden id="campaign-installments"
                                            value="{{ $order->orderCampaign->installments }}" />
                                        <input hidden id="campaign-code"
                                            value="{{ $order->orderCampaign->bt_payment_exp_code }}" />
                                        <input hidden id="order-remaining-amount"
                                            value="{{ $paidState['campaign_remaining_amount'] }}" />
                                    @else
                                        <input hidden id="order-remaining-amount"
                    ...
                    <div class="flex flex-col lg:flex-row gap-6 lg:justify-between border-gray-300 border-t-2 py-4">
                        <div class="flex flex-col items-start gap-4">
                            ...
                            <div class="flex items-center gap-2">
                                <x-bladewind.checkbox add_clearing="false" color="blue" id="remote_agreement"
                                    labelCss="mr-0" required="true" />
                                <span>{{ __('web.admit') }}</span> <span
                                    class="font-bold hover:text-blue-500 cursor-pointer"
                    ...
                    <div class="flex flex-col bg-white p-4 gap-2 rounded-md shadow-md">
                        ...
                        <div class="flex justify-between">
                            ...
                            <p class="font-bold text-[#0E60AE] text-right">
                                {{ $order->productVariation->getEstimatedDeliveryDate(isset($order->chasis_no)) }}</p>
                        </div>
                        <div class="flex justify-between">
    ...
    @section('js')
        <script>
            ...
            $("input[name=campaign]").on('change', function() {
                ...
                if (selectedValue) {
                    $("#oneshot_selection").attr('disabled', true);
                    $("#partial_selection").click();
                    const amount = $(this).attr('data-meta');
                    $("#custom_amount").val(amount);
                    $("#custom_amount").attr('readonly', true);
                    document.dispatchEvent(paymentAmountEvent);
                } else {
                    $("#oneshot_selection").attr('disabled', false);
                    $("#oneshot_selection").click();
                    $("#custom_amount").val(null);
                    $("#custom_amount").attr('readonly', false);
                    document.dispatchEvent(paymentAmountEvent);
                }
            })
        </script>
