...
<div
    class="relative px-2 py-3 border-2 border-dashed border-gray-500 text-center cursor-pointer rounded-md bw-fp-{{ $name }} @if ($add_clearing) mb-3 @endif">
    <x-bladewind::icon name="document-text"
        class="h-6 w-6 absolute z-20 left-4 rtl:!right-4 rtl:!left-[unset] text-gray-500" />
    <x-bladewind::icon name="x-circle"
        class="absolute right-3 rtl:!left-3 rtl:!right-[unset] h-8 w-8 text-gray-500 hover:text-gray-800 clear cursor-pointer hidden"
        type="solid" />
    <span class="text-gray-500 px-6 pl-10 zoom-out inline-block selection">
        {{ $placeholder }}
        @if ($required)
...
<script>
            ...
            if (file) {
                if (allowedFileSize(file.size, {{ $max_file_size }})) {
                    dom_el('.bw-fp-{{ $name }} .selection').innerHTML =
                        (file.type.indexOf('image') !== -1) ? '<img src="' + URL.createObjectURL(file) +
                        '" class="rounded-md" />' : file.name;
    ...
    @if (!empty($url))
        @if (in_array(pathinfo($url, PATHINFO_EXTENSION), $image_file_types))
            file =
                '<img src="{{ $url }}" alt="{{ $url }}" class="rounded-md {{ $selected_value_class }}" />';
        @else
